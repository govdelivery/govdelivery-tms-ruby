...
class CreateCommand < Thor
  ...
  def update(command_id)
    ...
    say "Updated command: #{command.id}", :green if command.save!
  end
  ...
  def actions(account_name, keyword_name)
    ...
    q = q.offset(options[:offset]) if options[:offset]
    puts q.to_sql
    q.each do |action|
  ...
  def get_account(account_name)
    Account.where(name: account_name).first || raise(Thor::Error.new("#{account_name} not found"))
  end

  def get_keyword(account, keyword_name)
    account.keywords.where(name: keyword_name).first || raise(Thor::Error.new("#{keyword_name} not found"))
  end

  def get_command(command_id)
    ::Command.find(command_id) || raise(Thor::Error.new("command: #{command_id} not found"))
  end
end
