...
File.exists?(gdconf) or raise GDConfMissing.new("cp config/vagrant_config_example.yml config/vagrant_example.yml or set VAGRANT_CONFIG,VAGRANT_USER_CONFIG")

CONFIG = YAML.load_file(gdconf)
if File.exists?(gduserconf)
  CONFIG.merge!(YAML.load_file(gduserconf) || {})
end
