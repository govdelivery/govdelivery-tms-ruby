...
describe Sidekiq::RateLimitedQueue do
  before do
    Sidekiq.options[:queues] = %w(sender default webhook stats low)
  ...
  it 'should merge queue lists' do
    expect(Sidekiq.options[:queues]).to match_array %w(sender sender_sadklfjhasdlkfjhasldkj default webhook stats low)
  end

  it 'should list throttled queues' do
    expect(Sidekiq::RateLimitedQueue.throttled_queues).to match_array %w(default sender_sadklfjhasdlkfjhasldkj)
  end
