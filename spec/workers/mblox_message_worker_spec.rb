...
describe MbloxMessageWorker do
  let(:sms_vendor) {create(:sms_vendor, worker: 'MbloxMessageWorker')}
  ...
  context 'a send' do
    it 'should create a batch job' do
      ...
      Mblox::SenderWorker.expects(:perform_async).with(message_id: message.id, recipient_id: recipient.id)

      subject.perform(message_id: message.id)
    end

    it 'should fail if not in sending state' do
      Mblox::SenderWorker.expects(:perform_async).never
